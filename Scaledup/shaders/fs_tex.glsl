...
uniform sampler2D u_texture;  //  sampler in the slides , u_texture is identifier of a texture object
uniform vec3 eyePos;
...
void main() {
    vec4 texcol = texture(u_texture, fs_uv);  // holds the value of the texture.
	...
	//diffuse
	vec4 diffuse = compDiffuse(lightDir, lightCol, normalVec, diffColor, eyedirVec);
